...
while True:
    ...
    def save_pic():
        ...
        #print(df.to_string())
        na=df[df["canceled time"]==1].count()[0]
        row=na//14
        col=na%14
        print(na)
        if (row>5):
            ctypes.windll.user32.MessageBoxW(0, "Maximum store gained", "maximum storage is gained please return when its free", 1)
        else:
            data = pd.DataFrame([[acc_name_text,str(now),str(now),1,str(row+1),str(col)]])
    ...
    def Use_Cell():
        ...
        if not exists:  
            ...
            if cell>3:
                ...
                tkinter.Button(text="Get Cell",command=get_cell_sto,font="Times 16 bold").place(x=600, y=650)
                tkinter.Button(text="Desolve Account",command=desolve_acc,font="Times 16 bold").place(x= 600, y=750)
